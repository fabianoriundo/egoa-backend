...
const LOGO_URL = 'https://imgur.com/a/K7hu9TM'; // Cambia por la URL real de tu logo
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // ── 3. Correo al administrador (TODOS los datos, diseño mejorado) ──────────
    ...
    // ── 4. Correo de confirmación al usuario (solo aviso, no datos sensibles) ───
    if (userEmail && userEmail.includes('@')) {
      await transporter.sendMail({
        from: `"EGOA Capital" <${process.env.SMTP_USER}>`,
        to: userEmail,
        subject: '✅ Recibimos tu solicitud de admisión · EGOA',
        html: buildUserEmail(nombre, userEmail),
      });
    } else {
...
// ---------- Plantilla para ADMIN (completa, con logo y diseño profesional) ----------
function buildAdminEmail(data: any, nombre: string): string {
  // Función auxiliar para mostrar campo con formateo
  const field = (label: string, value: any, highlight = false) => {
    if (!value || value === '') return '';
    const displayValue = Array.isArray(value) ? value.join(', ') : value;
    return `<tr>
      <td style="padding:10px 14px; background:#fff; border-bottom:1px solid #eef2f6; font-size:13px; color:#4b5563; width:35%">${label}</td>
  ...
  const sectionTitle = (title: string) => `
    <div style="margin-top:28px; margin-bottom:12px;">
      ...
      <h3 style="display:inline-block; font-size:15px; font-weight:700; color:#1f2937; margin:0; letter-spacing:-0.2px;">${title}</h3>
    </div>
  `;
  ...
  return `
    ...
    <body style="margin:0; padding:0; background:#f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
      <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:24px; overflow:hidden; box-shadow:0 10px 25px -5px rgba(0,0,0,0.05);">
        
        <!-- Encabezado con logo -->
        <div style="background:#0f172a; padding:28px 32px 24px; text-align:center;">
          <img src="${LOGO_URL}" alt="EGOA Capital" style="max-width:140px; margin-bottom:12px;" />
          <h1 style="color:#ffffff; font-size:20px; margin:12px 0 0; font-weight:600; letter-spacing:-0.3px;">Nueva solicitud de admisión</h1>
          <p style="color:#94a3b8; margin:4px 0 0; font-size:13px;">Revisión pendiente</p>
        </div>

        <!-- Cuerpo -->
        <div style="padding:24px 32px 32px;">
          <!-- Tarjeta de info del solicitante -->
          <div style="background:#f0f9ff; border-left:4px solid #1e40af; border-radius:12px; padding:16px 20px; margin-bottom:32px;">
            <p style="margin:0 0 4px; font-size:14px; font-weight:700; color:#1e3a8a;">${escapeHtml(nombre)}</p>
            <p style="margin:0; font-size:13px; color:#2563eb;">${data.email || 'Sin email'}</p>
          </div>

          <table style="width:100%; border-collapse:collapse; border-radius:16px; overflow:hidden; background:#fafbfc;">
            ...
            ${data.estadoCivil === 'casado' ? `
              ${sectionTitle('👫 Datos del cónyuge')}
              ${field('Nombre cónyuge', data.conyugeNombre)}
              ${field('Fecha nacimiento cónyuge', data.conyugeFecha)}
            ...
            ${sectionTitle('💼 Actividad profesional')}
            ...
            ${data.formaInversion === 'otro' ? field('Especificar forma inversión', data.formaInversionOtro) : ''}
            ...
            ${sectionTitle('🏡 Interés en copropiedad EGOA')}
            ...
            ${sectionTitle('💰 Rango de inversión')}
            ...
            ${sectionTitle('🏦 Compra y financiamiento')}
            ...
            ${sectionTitle('✍️ Declaración y envío')}
...
// ---------- Plantilla para USUARIO (solo confirmación, diseño atractivo) ----------
